...
def save_adovehicle_track(file_prefix, save_dir, file_counter, track_data, unique_id = None):
    """
    ...
    * 'timestamp': timestamps for the track points
    ...
    :return:
    """
    filename = os.path.join(save_dir,file_prefix) + '_' + ('' if unique_id is None else unique_id+'_') + '{0:08d}'.format(file_counter) + '.json'
    track_data['filehash'] = filename.split('/')[1]
    track_data['filename'] = filename.split('/')[2]
    with open(filename, mode='w') as fp:
        json.dump(track_data, fp,indent=4)
...
def normalize_track(track, normalizing_pose,is_normalizing=False):
    ...
    for i in range(len(res['heading_L'])):  #TODO: check once we have headings
...
def create_normalizing_pose(track,future_length,velocity_threshold = 4.0):
    ...
    # otherwise, throw away the example - rotation/pose is unclear. TODO: use heading

    for i in range(len(track['position_L']) - 1):
        pos0 = track['position_L'][-i - 1]
        velocity_v = np.array(pos1) - np.array(pos0)  # TODO: better estimate
        if np.linalg.norm(velocity_v) > velocity_threshold:
            break
    # import IPython;
    # IPython.embed(header='heading')
    if np.linalg.norm(velocity_v) < velocity_threshold:
        return None
